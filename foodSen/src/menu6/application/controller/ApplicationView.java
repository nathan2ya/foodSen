...
/*
 * 작성자: 류재욱
 * 설  명: 학교급식인력풀(구직) view 클래스
 * 용  도: 학교급식인력풀(구직) 게시글 뷰를 위함.
*/
...
public class ApplicationView {
	...
	//학교급식인력풀(구직) 뷰페이지
	@RequestMapping("/applicationView.do")
	public String inspectionResultView(HttpServletRequest request1, HttpSession session) throws SQLException{
		...
		sqlMapper.update("Application.updateHits", paramClass);
		...
		return "/view/menu6/recruit_application/applicationView.jsp";
	}
	
	
	//학교급식인력풀(구직) 첨부파일 다운로드
	@RequestMapping(value = "/application_FileDownload.do")
