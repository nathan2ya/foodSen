...
<script type="text/javascript">
	function goCreate(){
		...
		if(!applicationEditFrom.pw.value){
			alert("수정을 하시려면 비밀번호를 입력하세요.");
			applicationEditFrom.pw.focus();
			return;
		}
		
		if(oriPassword != applicationEditFrom.pw.value){
			alert("비밀번호를 다시 확인해주세요.");
			applicationEditFrom.pw.focus();
			return;
		}
		
		if(getStrByte(applicationEditFrom.description.value) > 1200){
			alert("내용은 2000자까지만 입력할 수 있습니다.");
			applicationEditFrom.description.value = applicationEditFrom.description.value.cut(1200);
			applicationEditFrom.description.focus();
			return;
		}
		
		if(validateSQL(applicationEditFrom.description.value) > -1){
...
<div id="container">
	<div id="contents">
		...
		<div class="right_box">
			...
			<!-- 게시판영역 -->
			<div class="tbl_box">
				<form name="applicationEditFrom" action="/foodSen/applicationEdit.do" method="post" enctype="multipart/form-data">
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbl_type01" summary="학교급식인력풀(구인)">
						...
						<tbody>
							...
							<tr>
								...
								<th>성별</th>
								<td colspan="1" class="tl">
									<c:if test="${resultClass.sex == 01}">
										남자
									</c:if>
									<c:if test="${resultClass.sex == 02}">
										여자
									</c:if>
								</td>
								<th>지역</th>
				...
				<span class="bbs_btn"> 
					...
					<c:if test="${searchingNow == 0}">
						<span class="wte_l"><a href="/foodSen/applicationList.do?currentPage=${currentPage}" class="wte_r">목록</a></span>
					</c:if>
					<!-- .//전체글 목록 -->
